...
後續所有需要身份資料的 API，auth-service 透過 `MemberContextFilter`（`common-biz` 自動配置）從 BFF 注入的 `X-Member-Id`、`X-Member-Account`、`X-Member-Role` header 取得，無需自行解析或查詢 Redis。詳見 `spec/common-biz/member-context.md`。
...
| accessToken | String | JWT Token | 包含於 auth-service Response Body，供 BFF 透過 Feign 讀取；不轉發給 Client（由 BFF 自己的 `LoginRs` 套用 `@JsonIgnore`） |
...
| 步驟 | 類型 | 說明 |
|------|------|------|
| 1 | `[DB READ]` | 透過 `MemberJpaRepository` 依 `account` 查詢 `member`；查無資料 → 拋出 `USER_NOT_FOUND` |
| 2 | `[DOMAIN]` | 檢查 `status == ACTIVE`；若否 → 拋出 `ACCOUNT_DISABLED` |
| 3 | `[DOMAIN]` | 驗證 `passwd` 與 DB 內 `password_hash` Bcrypt 雜湊值；不符 → 拋出 `INVALID_CREDENTIALS` |
| 4 | `[DOMAIN]` | 生成 JWT（claims：jti、sub=memberId、userAccount、role、iat、exp） |
| 5 | `[RETURN]` | 回傳 LoginRs（含 accessToken，供 BFF 透過 Feign 讀取） |
...
| password_hash | VARCHAR(255) | NO | — | Bcrypt 雜湊值 |
...
|-------------|------------|------|----------|
| 400 | USER_NOT_FOUND | 使用者不存在 | `account` 查無資料 |
| 400 | ACCOUNT_DISABLED | 帳號已停用 | `status != ACTIVE` |
| 400 | INVALID_CREDENTIALS | 密碼驗證失敗 | Bcrypt 比對不符 |
